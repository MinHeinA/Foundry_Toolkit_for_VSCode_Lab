# PersonalCareerCopilotCompleted

> [!IMPORTANT]
> This directory is the completed Careers MCP solution and trainer reference.
> Attendees create their own untracked `PersonalCareerCopilot/` project during
> Module 2 and use this directory only after attempting each task or during the
> trainer debrief.
...
```bash
cd workshop/lab02-multi-agent/PersonalCareerCopilotCompleted
...
Trainer reference deployment uses
[`../trainer-deployment/azure.yaml`](../trainer-deployment/azure.yaml). This
completed source intentionally has no `agent.yaml` or Dockerfile.
...
The attendee-generated project has its own manifest and source tree. Trainer
reference deployment/evaluation uses the isolated trainer service
`personal-career-copilot-reference`.

Follow the
[trainer deployment runbook](../trainer-deployment/README.md) from
`trainer-deployment/`. Attendees do not deploy or evaluate this reference
service.
