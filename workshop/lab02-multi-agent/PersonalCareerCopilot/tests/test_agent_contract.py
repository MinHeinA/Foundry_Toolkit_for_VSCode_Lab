...
STARTER_SOURCE = (ROOT.parent / "PersonalCareerCopilotStarter" / "main.py").read_text(
    encoding="utf-8"
)
...
def test_attendee_starter_is_runnable_baseline_not_completed_solution() -> None:
    ast.parse(STARTER_SOURCE)
    for number in range(1, 8):
        assert f"CAREERS MCP CHALLENGE TODO {number}" in STARTER_SOURCE
    assert "from careers_mcp import" not in STARTER_SOURCE
    assert "async def get_selected_careers_job" not in STARTER_SOURCE

    attendee_manifest = (ROOT.parent / "azure.yaml").read_text(encoding="utf-8")
    assert "project: PersonalCareerCopilotStarter" in attendee_manifest
